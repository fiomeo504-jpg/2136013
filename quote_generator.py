...
class QuoteGenerator:
    ...
    def create_widgets(self):
        ...
        tk.Label(add_frame, text="Текст цитаты:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.quote_text_entry = tk.Entry(add_frame, width=50)
        self.quote_text_entry.grid(row=0, column=1, padx=5, pady=5, columnspan=3)

        tk.Label(add_frame, text="Автор:").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.author_entry = tk.Entry(add_frame, width=25)
        self.author_entry.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(add_frame, text="Тема:").grid(row=1, column=2, sticky="e", padx=5, pady=5)
        ...
        tk.Button(add_frame, text="➕ Добавить цитату в библиотеку", command=self.add_quote,
                  bg="lightgreen").grid(row=1, column=4, padx=10, pady=5)

        # === Рамка генерации ===
        gen_frame = tk.LabelFrame(self.root, text="Генерация цитаты", padx=10, pady=10)
        ...
        self.current_quote_text = tk.Label(current_frame, text="", wraplength=700, font=("Georgia", 12, "italic"),
        ...
        tk.Label(filter_frame, text="Фильтр по автору:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.filter_author_combo = ttk.Combobox(filter_frame, width=20)
        self.filter_author_combo.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(filter_frame, text="Фильтр по теме:").grid(row=0, column=2, sticky="e", padx=5, pady=5)
        ...
        tk.Button(filter_frame, text="🔍 Применить фильтр", command=self.apply_filter,
                  bg="lightyellow").grid(row=0, column=4, padx=10, pady=5)
        tk.Button(filter_frame, text="❌ Сбросить фильтр", command=self.reset_filter,
                  bg="lightgray").grid(row=0, column=5, padx=5, pady=5)
